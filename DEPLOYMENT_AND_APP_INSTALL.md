...
```text
http://localhost:5173/index.html?v=19
```
...
Offline app shell:
`sw.js` caches the core app files, manifest, icons, logo, and offline fallback. It also ignores query-string versioning when matching cached assets, which helps `styles.v19.css` and `app.v19.js` resolve against the cached app shell. The production service worker is served with `Service-Worker-Allowed: /` and `Cache-Control: no-cache, no-store, must-revalidate`.
...
3. On the iPhone, open Safari to:
   ...
   ```text
   http://YOUR_MAC_LAN_IP:5173/index.html?v=19
   ```
...
- app shell references `styles.v19.css`
- app shell references `app.v19.js`
...
- production app shell references `styles.v19.css`
- production app shell references `app.v19.js`
