...
```text
http://localhost:5173/index.html?v=15
```
...
Offline app shell:
`sw.js` caches the core app files, manifest, icons, logo, and offline fallback. It also ignores query-string versioning when matching cached assets, which helps `styles.v15.css` and `app.v15.js` resolve against the cached app shell. The production service worker is served with `Service-Worker-Allowed: /` and `Cache-Control: no-cache, no-store, must-revalidate`.
...
3. On the iPhone, open Safari to:
   ...
   ```text
   http://YOUR_MAC_LAN_IP:5173/index.html?v=15
   ```
...
- app shell references `styles.v15.css`
- app shell references `app.v15.js`
...
- production app shell references `styles.v15.css`
- production app shell references `app.v15.js`
